...
public class ExceptionDemo1 {
	public static void main(String[] args) {
		try {
			...
			BufferedReader reader = new BufferedReader(new FileReader("memo123.txt"));
			
			// IPException을 던지는 메소드
			String text = reader.readLine();
			System.out.println("파일 내용" + text);
		} catch (FileNotFoundException fnfe) {
			System.out.println("예외발생!!!!");
			System.out.println("예외객체: " + fnfe);
			fnfe.printStackTrace(); // 오류룰 추적할 수 있는 정보를 화면에 출력하는 메소드;
			// 예외 처리 코드 작성
			// 1. 사용자 친화적인 에러 메세지를 표시한다.
			System.out.println("파일을 찾을 수 없습니다. 경로 및 파일명을 확인하세요.");
		} catch (IOException ioe) { //  읽기 쓰기 오류
